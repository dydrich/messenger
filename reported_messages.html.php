...
<head>
	...
	<title><?php echo $_SESSION['__config__']['intestazione_scuola'] ?>:: messaggi segnalati</title>
	...
	<script>
        ...
        var delete_msg = function(msid, status){
            //alert($('#targetID').val());
            $.ajax({
                type: "POST",
                url: "controller.php",
                data: {action: 'delete', mid: msid, tid: <?php echo $thread->getTid()?>, state: status},
                error: function() {
		...
		<div id="msgs" class="card_container" style="margin-top: 20px">
			...
			if (isset($reported) && count($reported) > 0){
				foreach ($reported as $k => $msg){
					$reason = null;
					if ($msg->getState() == Message::REPORTED_FOR_SPAM) {
						$reason = 'SPAM';
					}
					else {
                    ...
                    $state_on_delete = $msg->getState() + 2;
					...
					<div class="card" data-msg="<?php echo $msg->getID() ?>" style="<?php if(!$thread->isActive()) echo 'background-color: #EEEEEE' ?>">
						...
						<div class="card_varcontent" style="color: #1E4389; overflow: auto">
						    ...
						    <div style="width: 15%" class="fright">
							    ...
							    <div class="rb_button" style="width: 25px; height: 25px; top: -25px; margin-left: 40px">
								    <a href="#" class="remove" data-msg="<?php echo $msg->getID() ?>" data-state="<?php echo $state_on_delete ?>"
                                       title="Cancella il messaggio">
									    <i class="fa fa-trash" style="font-size: 1em; margin-top: 6px; margin-left: 7px; color: black"></i>
								    </a>
							    </div>
						    </div>
						</div>
